...
#     cv2.imshow('hihi', (cleaned*255).astype(uint8))
#     cv2.waitKey(-1)
    if args.usegauss:
        # this uses Gaussians
        ...
        grad = gaussian_filter(1.0*cleaned,(max(2,args.vscale*0.3*1.5*scale),
                                            args.hscale*1*scale),order=(1,0))   # <====== originally 1
        grad = uniform_filter(grad,(args.vscale,args.hscale*6*scale))   # <====== originally 6/35
...
#         print 'goc', angle
        ...
        h,w = img_grey.shape
        img_grey = cv2.normalize(img_grey.astype(float32), None, 0.2, 0.999, cv2.NORM_MINMAX)
...
#             hihi, sau = calc_line(line)
            ...
            if newwidth < 32 or newwidth > common.args.stdwidth: continue
